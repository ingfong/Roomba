...
class DiamondCollector(gym.Env):
    ...
    def __init__(self, env_config):  
        # Static Parameters
        self.size = 5
        self.diamond_reward_density = .1
        self.coal_reward_density = .2
        self.obs_size = 5
        ...
        def get_lava_and_diamonds():
            ...
            for x in range(n):
                for y in range (n):
                    ...
                    if random < self.diamond_reward_density:
                        blocks += "<DrawBlock x='{}'  y='2' z='{}' type='diamond_ore' />".format(x-self.size, y-self.size)
                    elif random < self.coal_reward_density:
                        blocks += "<DrawBlock x='{}'  y='2' z='{}' type='coal_ore' />".format(x-self.size, y-self.size)

            for y in range (-1,n+1):
                    blocks += "<DrawBlock x='{}'  y='1' z='{}' type='lava' />".format(self.size+1, y-self.size)
                    blocks += "<DrawBlock x='{}'  y='1' z='{}' type='lava' />".format(-self.size-1, y-self.size)

            for x in range (-1,n+1):
                    blocks += "<DrawBlock x='{}'  y='1' z='{}' type='lava' />".format(x-self.size, self.size+1)
                    blocks += "<DrawBlock x='{}'  y='1' z='{}' type='lava' />".format(x-self.size, -self.size-1)
            return blocks
